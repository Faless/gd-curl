...
# Add our build tools
for tool in ["openssl", "cmake", "mbedtls", "libzlib", "nghttp2", "nghttp3", "ngtcp2", "curl"]:
...
if use_openssl:
    ssl = env.OpenSSL()
    ngtcp2 = env.BuildNGTCP2(ssl)
    nghttp3 = env.BuildNGHTTP3()
elif use_mbedtls:
...
curl = env.BuildCURL(ssl, zlib, nghttp2, nghttp3, ngtcp2)
...
env.Depends(sources, ssl + zlib + curl + nghttp2 + nghttp3 + ngtcp2)
